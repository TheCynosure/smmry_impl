...
void tally_get_top_scorers(Bst* word_bst, LList *l, int return_num, Node* top_scorers[]);
...
void tally_get_top_scorers(Bst* word_bst, LList *l, int return_num, Node* top_scorers[]) {
    ...
    while(current_node != NULL) {
        ...
        for(c = current_node->data; *c != '\0'; c++) {
            if(*c == ' ') {
                /* Add score to our current sentence score if the word has appeared more than once */
